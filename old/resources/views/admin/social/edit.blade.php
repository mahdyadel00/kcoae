...
@section('content')
    ...
    <div id="content" class="main-content">
        <!--  BEGIN BREADCRUMBS  -->
        @include('admin.layouts.secondary_nav',['title'=>'التواصل الاجتماعي ','sub_title'=>'تعديل'])
        <br>
        <!--  END BREADCRUMBS  -->
                ...
                <div class="user-profile ">
                    <div class="widget-content widget-content-area">
                            <h3 class="mt-4"> </h3>
                                ...
                                <form class=" g-3" method="post" action="{{route('admin_panel.social.update',$social->id)}}" enctype="multipart/form-data" >
                                    @method('PATCH')
                                    @csrf
                                    @if ($errors->any())
...
{{--                                            <i class=" {{$social->icon}}"></i>--}}
                                            <i class=" {{$social->icon}}"></i>
                                            <button name="icon" class="btn btn-secondary" role="iconpicker" value="{{$social->icon}}"></button>
                                        </div>
                                        <div class="col-md-12 mt-4">
                                            <label for="inputEmail4" class="form-label">الرابط </label>
                                            <input type="url" class="form-control"  name="link" value="{{$social->link}}">
                                        </div>
